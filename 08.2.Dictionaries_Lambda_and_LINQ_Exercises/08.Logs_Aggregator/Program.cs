 ...
 * list of unique IP addresses in format "<user>: <duration> [<IP1>, <IP2>, …]". 
 ...
 * 
 * Output
 * Print one line for each user (order users alphabetically). For each user print its sum of durations and all 
 * of his sessions' IPs, ordered alphabetically in format <user>: <duration> [<IP1>, <IP2>, …]. 
...
namespace Logs_Aggregator
{
    class MainClass
    {
        public static void Main()
        {
            ...
            SortedDictionary<string, SortedDictionary<string, long>> data =
            ...
            for (int i = 0; i < logs; i++)
            {
                ...
                {
                    data[name] = new SortedDictionary<string, long>();
                    data[name][ipAddress] = duration;
                }
                else
                {
                    if (!data[name].ContainsKey(ipAddress))
                    {
                        data[name][ipAddress] = duration;
                    }
                    else
            ...
            foreach (var name in data)
            {
                ...
                {
                    if (ip.Key != name.Value.Keys.Last())
                    {
