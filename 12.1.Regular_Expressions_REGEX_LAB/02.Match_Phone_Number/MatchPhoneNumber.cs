...
namespace _02.Match_Phone_Number
{
    class Program
    {
        static void Main()
        {
            ...
            MatchCollection matchCollection = Regex.Matches(inputPhoneNumbers, pattern);

            List<string> resultPhones = new List<string>();

            foreach (Match number in matchCollection)
            {
                resultPhones.Add(number.ToString());
            }

            Console.WriteLine(string.Join(", ", resultPhones));
        }
    }
}
